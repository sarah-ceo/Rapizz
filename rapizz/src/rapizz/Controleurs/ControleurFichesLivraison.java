...
public class ControleurFichesLivraison {
	private User connectedUser;
	private static DecimalFormat df = new DecimalFormat("0.00");
	
	public ControleurFichesLivraison(FenetreAccueilAdmin f1, FenetreFichesLivraisons f2, User user, DAOCommande daocommande, DAOPizza daopizza) {
		this.connectedUser = user;
		
		List<Commande> commandesObj = daocommande.getAllCommandes();
		...
		f2.getBtnActualiser().addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (commandeIndex>-1) {
					...
					f2.getTextPaneClient().setText(daocommande.getNomClient(commandeSelectionnee.getIdCommande()));
					f2.getTextPaneLivreur().setText(daocommande.getNomLivreur(commandeSelectionnee.getIdCommande()));
					f2.getTextPaneVehicule().setText(daocommande.getNomVehicule(commandeSelectionnee.getIdCommande()));
